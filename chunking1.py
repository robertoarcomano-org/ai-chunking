...
class ParagraphAwareChunker:
    ...
    def _prendi_overlap(self, testo_precedente: str) -> str:
        """Estrae l'ultima porzione del chunk precedente (fino a
        chunk_overlap caratteri) per usarla come contesto di raccordo,
        senza spezzare la parola a metà."""
        if not testo_precedente or self.chunk_overlap <= 0:
            return ""
        coda = testo_precedente[-self.chunk_overlap:]
        # Allinea all'inizio della parola successiva per non tagliarla
        spazio = coda.find(" ")
        return coda[spazio + 1:] if spazio != -1 else coda
